...
public class Game implements Runnable, KeyListener {
	...
	public static final Dimension DIM = new Dimension(1000, 700); //the dimension of the game.
	...
	private final int PAUSE = 80, // p key
			...
			DOWN = 40, //back thrust; down arrow
			START = 83, // s key
			FIRE = 32, // space key
			MUTE = 77, // m-key mute
	...
	// NUM_ENTER = 10, 				// hyp
			...
			SPECIAL2 = 90;			// fire second special weapon; Z key
		...
		if (CommandCenter.getInstance().getFalcon() != null){
			...
			for (Movable movFloater : CommandCenter.getInstance().getMovFloaters()) {
				...
				if (pntFalCenter.distance(pntFloaterCenter) < (nFalRadiux + nFloaterRadiux)) {
					CommandCenter.getInstance().addNumFalcons();
	...
	private void killFoe(Movable movFoe) {
		...
		if (movFoe instanceof Asteroid){
			...
			if(astExploded.getSize() == 0){
				...
				CommandCenter.getInstance().getOpsList().enqueue(new Asteroid(astExploded), CollisionOp.Operation.ADD);
				CommandCenter.getInstance().setScore(5);
			...
			//medium size asteroid exploded
				...
				CommandCenter.getInstance().getOpsList().enqueue(new Asteroid(astExploded), CollisionOp.Operation.ADD);
				CommandCenter.getInstance().getOpsList().enqueue(new Asteroid(astExploded), CollisionOp.Operation.ADD);
				CommandCenter.getInstance().setScore(10);
				...
				CommandCenter.getInstance().setScore(20);
			}
		} 
	...
	private void spawnNewShipFloater() {
		...
		int floaterNum = nTick % (SPAWN_NEW_SHIP_FLOATER - nLevel * 7);

		if (floaterNum == 0 || floaterNum == 5 || floaterNum == 7) {
	...
	public void keyReleased(KeyEvent e) {
		...
		if (fal != null) {
			...
			case SPECIAL2:
				CommandCenter.getInstance().getOpsList().enqueue(new LaserWave(fal, true), CollisionOp.Operation.ADD);
				CommandCenter.getInstance().getOpsList().enqueue(new LaserWave(fal, false), CollisionOp.Operation.ADD);
				Sound.playSound("laserwave.wav");
				break;
