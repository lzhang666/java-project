...
public class Falcon extends Sprite {
	...
	private boolean bBackFlame = false; //add back flame

	private boolean bProtected; //for fade in and out
	
	private boolean bThrusting = false;
	private boolean bBackThrusting = false; //backward thrusting
	...
	//back thrusting begin
	public void backThrustOn() {
		bBackThrusting = true;
	}
	public void backThrustOff() {
		bBackThrusting = false;
		bBackFlame = false;
	}
	//back thrusting end
	...
	public void draw(Graphics g) {
		...
		if (getFadeValue() == 255) {
			...
			colShip = new Color(adjustColor(getFadeValue(), 200), getFadeValue(), adjustColor(getFadeValue(), 100));
		}
		...
		if (bFlame) {
			...
			for (int nC = 0; nC < FLAME.length; nC++) {
				if (nC % 2 != 0) //odd
				{
					pntFlames[nC] = new Point((int) (getCenter().x + 2.5 * getRadius() * Math.sin(Math.toRadians(getOrientation()) + FLAME[nC])),
							(int) (getCenter().y - 2.5 * getRadius() * Math.cos(Math.toRadians(getOrientation()) + FLAME[nC])));

				} else //even
				{
					pntFlames[nC] = new Point((int) (getCenter().x +
							getRadius() * 1.1 * Math.sin(Math.toRadians(getOrientation()) + FLAME[nC])),
		...
		if (bBackFlame) {
			...
			for (int nC = 0; nC < FLAME.length; nC++) {
				if (nC % 2 != 0) //odd
				{
					pntFlames[nC] = new Point((int) (getCenter().x - 2 * getRadius() * Math.sin(Math.toRadians(getOrientation()) + FLAME[nC])),
							(int) (getCenter().y + 2 * getRadius() * Math.cos(Math.toRadians(getOrientation()) + FLAME[nC])));

				} else //even
				{
					pntFlames[nC] = new Point((int) (getCenter().x + getRadius() * (-1.1) * Math.sin(Math.toRadians(getOrientation()) + FLAME[nC])),
							(int) (getCenter().y - getRadius() * (-1.1) * Math.cos(Math.toRadians(getOrientation()) + FLAME[nC])));
